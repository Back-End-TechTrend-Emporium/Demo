    ...
    _logger = logger;
        ...
        public async Task<ProductDto> CreateProductAsync(ProductCreateDto productDto, Guid createdBy)
        {
            try
            {
                ...
                product.State = ApprovalState.PendingApproval; // Productos manuales necesitan aprobaci�n
        ...
        public async Task<ProductDto?> UpdateProductAsync(Guid id, ProductUpdateDto productDto)
        {
            try
            {
                ...
                // Si se cambia la categor�a, buscar o crear la nueva
        ...
        public async Task<int> SyncAllFromFakeStoreAsync(Guid createdBy)
        {
            try
            {
                _logger.LogInformation("Iniciando sincronizaci�n completa desde FakeStore");
                ...
                _logger.LogInformation("Sincronizaci�n completada: {Count} productos importados", importedCount);
                return importedCount;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error en sincronizaci�n completa");
                throw;
            }
        }
        ...
        private async Task<Category> GetOrCreateCategoryAsync(string categoryName, Guid createdBy)
        {
            ...
            _logger.LogInformation("Categor�a creada: {CategoryName}", categoryName);
            return category;
        }
        ...
        private static ProductDto MapToProductDto(FakeStoreProductResponse fakeStoreProduct)
        {
            ...
            {
                Id = Guid.NewGuid(), // Generar GUID �nico para productos de FakeStore
                Title = fakeStoreProduct.Title,
                Price = fakeStoreProduct.Price,
                Description = fakeStoreProduct.Description,
